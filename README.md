...
| Theme system | **Dark/cinematic for moment screens** (roll, reveal, camera, entry logging) **, light/strict-grayscale for utility screens** (accounts, profile, diary, board, settings) | Based on the user's own moodboard reference, which independently split the same way — drama for the ritual, clarity for the everyday app, not an arbitrary global light-or-dark pick. Phase 1 (accounts/profile) is entirely on the light/utility side. |
...
| Phase 1 palette | **Strict grayscale** — white, near-black `#262626`, gray surfaces/text, with muted red/green kept only as small functional signals (errors, username-available) | User: "keep it all black and white... how Instagram does." Matches the user's own moodboard color panel (black/white/gray) closely. UI-SPEC revision 5, checker-approved. |
