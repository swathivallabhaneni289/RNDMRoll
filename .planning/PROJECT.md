...
- **Moodboard reference (2026-09-15):** user provided two reference mockup boards for "rndmroll" establishing overall visual direction. Extracted as project-wide reference, not literally copied (user's explicit framing: "similar vibe... not the exact same thing... make sure it's not really too much ai coded" — the board's own bottom design-system panel listed Playfair Display + Inter as its typefaces, which are two of the most recognizable AI-generated-UI default fonts, so exact fonts/colors were deliberately not copied verbatim):
  - **Theme system, whole-app:** dark/cinematic for "moment" screens (daily roll/spin, reveal, camera capture, new-entry logging — Phase 2 & Phase 3), light for "utility" screens (accounts/profile — Phase 1; diary, friend board, wheel settings — Phase 5). This mirrors the actual light/dark split observed across the moodboard's own screens (its dark screens are exactly the moment screens; its light screens are exactly the utility screens), not an arbitrary 50/50 split. Phase 1's UI-SPEC (light, strict grayscale) already conforms — no change needed there. Phase 2/3 UI-SPECs should establish the dark/cinematic treatment when those phases are designed.
...
| Theme system: dark/cinematic for moment screens (roll, reveal, camera, entry), light/grayscale for utility screens (accounts, profile, diary, board, settings) | Matches the user's moodboard's own actual light/dark split; drama for the ritual, clarity for the everyday app — not an arbitrary global light-or-dark choice | — Pending |
